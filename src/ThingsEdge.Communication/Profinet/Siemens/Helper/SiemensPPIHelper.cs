...
/// 西门子PPI协议的辅助类对象
/// </summary>
public static class SiemensPPIHelper
{
    ...
    /// <param name="address">起始地址，例如M100，I0，Q0，V100</param>
    ...
    public static bool CheckReceiveDataComplete(MemoryStream ms)
    {
        ...
        if (array.Length > 6 && array[0] == 104 && array[1] + 6 == array.Length && array[^1] == 22)
        {
            return true;
        }
    ...
    public static async Task<OperateResult<byte[]>> ReadAsync(IReadWriteDevice plc, string address, ushort length, byte station, AsyncLock asyncLock)
    {
        ...
        return await ReadAsync(plc, operateResult.Content, length, station2, asyncLock).ConfigureAwait(false);
    }
    ...
    public static async Task<OperateResult<string>> ReadPlcTypeAsync(IReadWriteDevice plc, string parameter, byte station, AsyncLock asyncLock)
    {
        ...
        using (await asyncLock.LockAsync())
        {
            ...
            try
            {
                ...
                {
                    Array.Copy(operateResult3.Content, 25, array, 0, 20); // Think: 优化，不用拷贝
                }
    ...
    public static OperateResult<S7AddressData> AnalysisAddress(string address)
    {
        ...
        try
        {
            ...
            if (address.StartsWith("SYS"))
            {
                ...
                s7AddressData.AddressStart = S7AddressData.CalculateAddressStarted(address[3..]);
            }
            else if (address.StartsWith("AI"))
            {
                s7AddressData.DataCode = 6;
                s7AddressData.AddressStart = S7AddressData.CalculateAddressStarted(address[2..]);
            }
            ...
            else if (address[0] == 'T')
            {
                ...
                s7AddressData.AddressStart = S7AddressData.CalculateAddressStarted(address[1..]);
            }
            else if (address[0] == 'C')
            {
            ...
            else if (address[0] == 'D' || address.StartsWith("DB"))
            {
                ...
                s7AddressData.AddressStart = S7AddressData.CalculateAddressStarted(address[(address.IndexOf('.') + 1)..]);
            }
            else
            {
                if (address[0] != 'V')
    ...
    private static string GetMsgFromStatus(byte code)
    {
        return code switch
        {
    ...
    private static string GetMsgFromStatus(byte errorClass, byte errorCode)
    {
