...
public class AllenBradleyNet : DeviceTcpNet, IReadWriteCip, IReadWriteNet
{
    ...
    {
        var read = await ReadFromCoreServerAsync(CommunicationPipe, AllenBradleyHelper.RegisterSessionHandle(), true, false).ConfigureAwait(false);
        if (!read.IsSuccess)
        {
            return read;
        }
        ...
        if (MessageRouter != null)
        {
            ...
            var messageRouter = await ReadFromCoreServerAsync(CommunicationPipe, send, true, false).ConfigureAwait(false);
