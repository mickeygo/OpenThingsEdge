...
public static partial class PayloadDataExtensions
{
    ...
    /// </remarks>
    public static bool TryGetAsBoolean(this PayloadData payload, [MaybeNullWhen(false)] out bool? value)
    {
    ...
    /// </remarks>
    public static bool TryGetAsInt32(this PayloadData payload, [MaybeNullWhen(false)] out int? value)
    {
    ...
    /// </remarks>
    public static bool TryGetAsDouble(this PayloadData payload, [MaybeNullWhen(false)] out double? value)
    {
        switch (payload.DataType)
        {
            case TagDataType.Bit:
    ...
    /// 仅将原始类型 Bit 转换为 Boolean 类型。
    /// </remarks>
    public static bool TryGetAsBooleanArray(this PayloadData payload, [MaybeNullWhen(false)] out bool[]? value)
    {
        if (payload.IsArray() && payload.DataType == TagDataType.Bit)
        {
            value = ObjectConvertUtil.ToBooleanArray(payload.Value);
            return true;
        }
    ...
    /// 会将原始类型 Bit、Byte、Word、Int 和 DInt 转换为 Int32 类型。
    /// </remarks>
    public static bool TryGetAsInt32Array(this PayloadData payload, [MaybeNullWhen(false)] out int[]? value)
    {
        if (payload.IsArray())
        {
            ...
            {
                value = ObjectConvertUtil.ToArray(payload.Value, Convert.ToInt32);
                return true;
            }
        }
    ...
    /// 会将原始类型 Bit、Byte、Word、DWord、Int、DInt、Real 和 LReal 转换为 double 类型。
    /// </remarks>
    public static bool TryGetAsDoubleArray(this PayloadData payload, [MaybeNullWhen(false)] out double[]? value)
    {
        if (payload.IsArray())
        {
            ...
            {
                value = ObjectConvertUtil.ToArray(payload.Value, Convert.ToDouble);
                return true;
            }
        }
