    ...
    ILogger<HeartbeatWorker> logger) : IWorker
{
    public Task RunAsync(IDriverConnector connector, string channelName, Device device, CancellationToken cancellationToken)
    {
        ...
        var tags = device.GetAllSignalTags(TagFlag.Heartbeat); // 所有标记为心跳的都进行监控
        foreach (var tag in tags)
        {
            _ = Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    // 任务取消时，发布设备心跳断开事件。
                    if (cancellationToken.IsCancellationRequested && TagDataAccesstor.CompareAndExchange(tag.TagId, false))
                    {
                        try
                        {
                            await broker.PushAsync(
